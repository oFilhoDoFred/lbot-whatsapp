// ARQUIVO: events/group-participants-updated.event.js

import path from 'node:path';
// AQUI ESTÁ A CORREÇÃO: Adicionamos 'randomDelay' na lista de importação
import { buildText, showConsoleError, randomDelay } from '../utils/general.util.js';
...
async function sendWelcome(client, group, botInfo, userId) {
    if (group.welcome.status) {
        // --- MENSAGEM 1: IMAGEM ---
        const imagePath = path.resolve('dist/media/welcome.gif');
        const customMessage = group.welcome.msg ? group.welcome.msg + "\n\n" : "";
        const welcomeCaption = buildText(
        ...
        // --- MENSAGEM 2: TEXTO COM REGRAS ---
        const textoRegras = botTexts.welcome_second_message;
        await client.sendMessage(group.id, { text: textoRegras });
    }
}