// ARQUIVO: commands/utility.functions.commands.js (VERSÃO ATUALIZADA)
// (Todo o restante do arquivo permanece igual, a única mudança é na função climaCommand)
...
// ... (todas as outras funções como ouvirCommand, noticiasCommand, etc. ficam aqui)
export async function ouvirCommand(client, botInfo, message, group) {
    ...
    else if (message.quotedMessage?.media?.seconds && message.quotedMessage?.media?.seconds > 120) {
...
export async function noticiasCommand(client, botInfo, message, group) {
    try {
        await waUtil.replyText(client, message.chat_id, "⏳ Buscando e processando as últimas notícias, isso pode levar um momento...", message.wa_message, { expiration: message.expiration });
        const newsList = await miscUtil.newsGoogle();
        if (!newsList || newsList.length === 0) {
            await waUtil.replyText(client, message.chat_id, "❌ Nenhuma notícia encontrada no momento.", message.wa_message, { expiration: message.expiration });
            return;
        }
        const top10News = newsList.slice(0, 10);
        let replyText = utilityCommands.noticias.msgs.reply_title;
        for (const news of top10News) {
            const shortUrl = await miscUtil.shortenUrl(news.url);
            replyText += buildText(utilityCommands.noticias.msgs.reply_item, news.title, news.author, news.published, shortUrl);
        }
        await waUtil.replyText(client, message.chat_id, replyText, message.wa_message, { expiration: message.expiration });
    } catch (error) {
        console.error("Erro no comando !noticias:", error);
        await waUtil.replyText(client, message.chat_id, "❌ Desculpe, ocorreu um erro ao buscar as notícias.", message.wa_message, { expiration: message.expiration });
    }
}
...
export async function climaCommand(client, botInfo, message, group) {
    ...
    try {
        // 1. Busca os dados do clima (já configurado para 3 dias no misc.util.js).
        let weatherResult = await miscUtil.wheatherInfo(message.text_command);
        if (!weatherResult) {
            throw new Error("Não foi possível obter os dados do clima.");
        }

        // 2. Pega os dados de hoje (índice 0) e amanhã (índice 1) da previsão.
        const previsaoHoje = weatherResult.forecast[0];
        const previsaoAmanha = weatherResult.forecast[1];

        // 3. Monta a seção do "Clima Atual".
        let replyText = buildText(
            utilityCommands.clima.msgs.reply,
            message.text_command,
            weatherResult.location.name,
            weatherResult.location.region,
            weatherResult.location.country,
            weatherResult.location.current_time,
            weatherResult.current.temp,
            weatherResult.current.feelslike,
            weatherResult.current.condition,
            weatherResult.current.wind,
            weatherResult.current.humidity,
            weatherResult.current.cloud
        );

        // 4. Monta e adiciona a seção "Previsão para Hoje".
        if (previsaoHoje) {
            replyText += buildText(
                utilityCommands.clima.msgs.reply_forecast,
                `Hoje (${previsaoHoje.day})`, // Adiciona o texto "Hoje"
                previsaoHoje.max,
                previsaoHoje.min,
                previsaoHoje.condition,
                previsaoHoje.max_wind,
                previsaoHoje.chance_rain,
                previsaoHoje.rain, // Parâmetro {7} para chuva? Sim/Não
                previsaoHoje.uv
            );
        }

        // 5. Monta e adiciona a seção "Previsão para Amanhã".
        if (previsaoAmanha) {
            replyText += buildText(
                utilityCommands.clima.msgs.reply_forecast,
                `Amanhã (${previsaoAmanha.day})`, // Adiciona o texto "Amanhã"
                previsaoAmanha.max,
                previsaoAmanha.min,
                previsaoAmanha.condition,
                previsaoAmanha.max_wind,
                previsaoAmanha.chance_rain,
                previsaoAmanha.rain, // Parâmetro {7} para chuva? Sim/Não
                previsaoAmanha.uv
            );
        }
        ...
        await waUtil.replyText(client, message.chat_id, `❌ Desculpe, não encontrei informações do clima para "${message.text_command}".`, message.wa_message);
    }
}
