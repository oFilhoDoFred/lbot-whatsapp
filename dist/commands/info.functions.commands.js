...
// DENTRO DO ARQUIVO: info.functions.commands.js

export async function menuCommand(client, botInfo, message, group) {
    // 1. Montamos um novo cabeçalho, APENAS com a saudação e o separador.
    // Usamos o "message.pushname", que já temos, sem precisar de buscas extras.
    let replyText = `Olá, *${message.pushname}*!\n` +
                    `|───────────────────────────────────────────\n`;

    // 2. O restante do código continua o mesmo. Ele vai adicionar o texto do menu
    // logo após o cabeçalho que acabamos de criar.
    if (!message.args.length) {
    ...
    // 3. O texto final, com o cabeçalho personalizado e o menu, é enviado.
    await waUtil.replyText(client, message.chat_id, replyText, message.wa_message, { expiration: message.expiration });
}
